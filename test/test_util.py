...
def test_optimizer_distribution():
    ...
    rv = OptimizerDistribution(gd={"step_rate": [0.1, 0.2], "momentum": [0.9, 0.99]})
    opt = rv.rvs()
    assert opt[0] == "gd"
    assert opt[1]["step_rate"] in [0.1, 0.2]
    assert opt[1]["momentum"] in [0.9, 0.99]

    rv = OptimizerDistribution(
        gd={"step_rate": [0.1, 0.2], "momentum": [0.9, 0.99]}, lbfgs={"n_factors": [10, 25]}
    )
    opt = rv.rvs()
