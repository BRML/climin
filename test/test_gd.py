...
def test_gd_quadratic():
    obj = Quadratic()
    opt = GradientDescent(obj.pars, obj.fprime, step_rate=0.01, momentum=0.9)
...
def test_gd_lr():
    ...
    opt = GradientDescent(obj.pars, obj.fprime, step_rate=0.01, momentum=0.9, args=args)
    for i, info in enumerate(opt):
        if i > 500:
            break
    assert obj.solved(), "did not find solution"
...
def test_gd_quadratic_nesterov():
    obj = Quadratic()
    opt = GradientDescent(
        obj.pars, obj.fprime, step_rate=0.01, momentum=0.9, momentum_type="nesterov"
    )
    for i, info in enumerate(opt):
        if i > 500:
            break
...
def test_gd_lr_nesterov():
    ...
    args = itertools.repeat(((obj.X, obj.Z), {}))
    opt = GradientDescent(
        obj.pars, obj.fprime, step_rate=0.01, momentum=0.9, momentum_type="nesterov", args=args
    )
    for i, info in enumerate(opt):
        if i > 500:
            break
